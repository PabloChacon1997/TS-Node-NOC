...
describe('FileSystemDatasource', () => {
  ...
  test('should save a log in logs-all.log', () => {
    ...
    const log = new LogEntity({
      ...
      origin: 'file-system.datsdource.test.ts'
    });
  ...
  test('should not throw a error if path exists', () => {
    new FileSystemDatasource();
    new FileSystemDatasource();
  });
  test('should throw an error if severity level is not defined', async () => {
    const logDatasource = new FileSystemDatasource();
    const cusmtomSeverityLevel = 'SUPER_MEGA_HIGH' as LogSeverityLevel;
    try {
      await logDatasource.getLogs(cusmtomSeverityLevel);
      expect(true).toBeFalsy();
    } catch (error) {
      const errorString = `${error}`;
      expect(errorString).toContain(`${cusmtomSeverityLevel} not implemented`)
    }
  });
});